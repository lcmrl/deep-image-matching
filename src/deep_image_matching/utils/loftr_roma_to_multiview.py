...
def import_into_colmap(
    ...
    fname_to_id = add_keypoints(
        db, feature_dir, img_dir, camera_model=camera_model, single_camera=single_camera
    )
    add_matches(
        db,
        feature_dir,
        ...
        for img1 in h5_matches.keys():
            ...
            for img2 in group_match.keys():
                ...
                h5_out[img1][img2] = np.hstack(
                    (kpts1[matches[:, 0], :], kpts2[matches[:, 1], :])
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LOFTR and Roma matchers to multi-view. Assign same index to keypoints in different images with distance < 1 px"
    )
    parser.add_argument(
        "-i",
        "--input_dir",
        type=str,
        help="Path to directory containing databases features.h5 and matches.h5",
    )
    parser.add_argument("-o", "--output_dir", type=str, help="Output directory")
    parser.add_argument("-d", "--image_dir", type=str, help="Image directory")
    parser.add_argument(
        "-e", "--img_ext", type=str, default=".jpg", help="Image extension"
    )
